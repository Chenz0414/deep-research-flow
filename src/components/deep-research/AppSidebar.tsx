...
export function AppSidebar({ sessions, activeSessionId, onNewResearch, onSelectSession, mobileOpen: controlledMobileOpen, onMobileOpenChange }: AppSidebarProps) {
  ...
  const sidebarContent = (
    <div className="h-full flex flex-col bg-sidebar border-r border-border/50 w-full sm:w-[260px]">
      ...
      <div className="p-5 flex-shrink-0 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={toggleTheme}
              className="p-1.5 rounded-md hover:bg-sidebar-hover transition-colors cursor-pointer"
            ...
            <button
              ...
              className="hidden sm:flex p-1.5 rounded-md hover:bg-sidebar-hover transition-colors cursor-pointer"
              title="收起侧边栏"
            >
              <PanelLeftClose className="w-4 h-4 text-muted-foreground" />
            </button>
            <button onClick={() => setMobileOpen(false)} className="sm:hidden p-1 rounded-md hover:bg-sidebar-hover transition-colors cursor-pointer">
      ...
      <div className="px-4 pb-3 flex-shrink-0">
        <div className="relative">
          ...
          <input
            ...
            placeholder="搜索历史记录..."
            className="w-full pl-9 pr-3 py-2 text-xs rounded-lg border border-border/60 bg-muted text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/40 transition-colors"
          />
        </div>
      </div>
      ...
      <ScrollArea className="flex-1 px-3">
        <div className="space-y-1 pb-4">
          {filtered.length === 0 ? (
            <p className="text-xs text-muted-foreground px-3 py-8 text-center">暂无记录</p>
          ) : (
                ...
                >
                  <div className="flex items-start gap-2.5">
                    {session.stage === 'COMPLETED' ? (
                      ...
                      <FileText className="w-4 h-4 mt-0.5 flex-shrink-0 text-muted-foreground group-hover:text-foreground transition-colors" />
                    )}
                    <div className="min-w-0 flex-1">
                      <p className={`truncate font-medium ${isActive ? 'text-foreground' : 'text-muted-foreground'}`}>{session.title}</p>
    ...
    <>
      {onMobileOpenChange == null && (
        <button
          ...
          className="fixed top-4 left-4 z-50 sm:hidden p-2 rounded-lg bg-card border border-border/60 shadow-sm hover:bg-accent transition-colors cursor-pointer"
        >
